...
export class FirebaseAuthService extends BaseService implements IAuthService {
  ...
  async sendPasswordResetEmail(
    ...
    try {
      const firebaseLink = await this.auth.generatePasswordResetLink(email);
      const oobCode = new URL(firebaseLink).searchParams.get('oobCode');
      resetLink = `${appConfig.frontendUrl}/reset-password?oobCode=${oobCode}`;
    } catch (error: any) {
      if (error?.code === 'auth/user-not-found') {
        // Don't reveal whether the account exists.
        return genericResult;
      }
      throw new InternalServerError('Failed to generate password reset link');
    }

    await this.mailService.sendMail({
      to: email,
      subject: 'Reset your ViBe password',
      text: `Someone requested a password reset for your ViBe account.\n\nReset your password: ${resetLink}\n\nIf you didn't request this, you can safely ignore this email.`,
      html: `<!DOCTYPE html>
<html lang="en" xmlns="http://www.w3.org/1999/xhtml">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Reset your ViBe password</title>
</head>
    ...
    <tr>
      <td align="center">
        <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="background-color:#ffffff; border-radius:8px; overflow:hidden;">
          ...
          <tr>
            <td style="padding:0 32px 24px; font-family:Arial, sans-serif; color:#333333; font-size:16px; line-height:24px;">
              <p>Hello,</p>
              <p>Someone requested a password reset for your ViBe account (<strong>${email}</strong>). Click the button below to choose a new password.</p>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td style="padding:0 32px 32px; font-family:Arial, sans-serif; color:#888888; font-size:13px; line-height:20px;">
